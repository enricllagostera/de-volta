...
func _process(delta):
	if $MapCamera2D.current:
		$MapCamera2D.offset = lerp($MapCamera2D.offset, map_camera_target, delta * map_camera_smooth)
...
func _on_NavHSlider_value_changed(value):
	var new_x = range_lerp(value, 0, 100, $MapCamera2D.limit_left, $MapCamera2D.limit_right - OS.window_size.x*$MapCamera2D.zoom.x)
	map_camera_target.x = new_x


func _on_NavVSlider_value_changed(value):
	var new_y = range_lerp((100-value), 0, 100, $MapCamera2D.limit_top, $MapCamera2D.limit_bottom - OS.window_size.y*$MapCamera2D.zoom.y)
